...
```
```{r}
cov.tree <- read.tree("~/Desktop/working_phy_pruned.tre")
cl_lv.data <- read.csv("~/Desktop/cl_lv_cv.csv")
...
#object<-plotTree.datamatrix(cl_lv.tree,cl_lv_cv.data,
	#fsize=0.5,yexp=1,header=FALSE,
	#palettes=c("YlOrRd","PuBuGn"))
```

```{r}
#leg<-legend(x="topleft",names(object$colors$cleavage_state),
	#cex=0.7,pch=22,pt.bg=object$colors$cleavage_state,
	#pt.cex=1.5,bty="n",title="cleavage state")
## add a second legend for trait 2
#leg<-legend(x=leg$rect$left+4.7,y=leg$rect$top-leg$rect$h,
	#names(object$colors$larval_type),cex=0.7,
	#pch=22,pt.bg=object$colors$larval_type,pt.cex=1.5,
  #bty="n",title="larval type")
...
mtext(paste("b) larval type depends on cleavage state",
	"\n  (but not the reverse), AIC =",
...
```{r}
#layout(matrix(c(1,1,1,2,3,4),3,2))
#object<-plotTree.datamatrix(cl_lv.tree,cl_lv_final.data,
	#fsize=0.5,yexp=1,header=FALSE,
	#palettes=c("YlOrRd","PuBuGn"))
```
...
```{r}
es_lv.data <- read.csv("~/Desktop/es_lv_cv.csv")
```
